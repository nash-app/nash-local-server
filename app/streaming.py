...
class StreamProcessor:
        ...
        if hasattr(chunk, "choices") and chunk.choices:
            ...
            if self.collecting_tool_call and self.finish_reason:
                ...
                try:
                    ...
                    self.tool_use_info = {"id": self.tool_call_id, "name": self.tool_name, "input": parsed_args}
    ...
    def get_message_for_history(self):
        ...
        if self.is_tool_call_detected():
            ...
            if self.assistant_message:
                return {"role": "assistant", "content": self.assistant_message + tool_call_text}
            else:
                return {"role": "assistant", "content": tool_call_text}
    ...
    async def execute_tool(self, mcp_handler):
        ...
        if not self.is_tool_call_detected():
            return {"success": False, "result_text": "No tool call detected", "result_message": None}

        try:
            # Execute the tool with the parsed arguments
            tool_result = await mcp_handler.call_tool(self.tool_use_info["name"], arguments=self.tool_use_info["input"])
            ...
            return {"success": True, "result_text": result_text, "result_message": result_message}
            ...
            return {"success": False, "result_text": f"Error: {error_message}", "result_message": result_message}
